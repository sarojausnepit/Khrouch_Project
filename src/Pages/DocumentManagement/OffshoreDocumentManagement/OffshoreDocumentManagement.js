...
const OffshoreDocumentManagement = () => {
  const offshoreDocumentState = useSelector(
  ...
  const fetchOffshoreDocuments = () => {
    actions(
      allActions(
        {},
        {
          method: "get",
          endPoint: `/course/checkList/get-all/offshore?page=${page}&pageSize=${pageSize}`,
  ...
  const data = offshoreDocumentState.offshoreDocuments?.data?.map((dataObj) => {
    return {
      key: dataObj.id,
      id: dataObj.id,
  ...
  console.log("from offshoreDocument state", offshoreDocumentState);
  return (
    <SiderDemo>
      <div className="flexColumnwithoutStyle">
        <div className="flexRow">
          ...
          >
            <span>Add Offshore Document</span>
          </button>
        </div>
        <div
          style={{
        ...
        >
          {offshoreDocumentState.isLoading ? (
            <TableSkeleton />
          ) : (
            <Table bordered columns={columns} dataSource={data} />
          )}

          {offshoreDocumentState.offshoreDocuments?.totalData && (
            <Pagination
              style={{
                marginTop: "1rem",
                marginBottom: "1rem",
              ...
              total={offshoreDocumentState.offshoreDocuments?.totalPage * 10}
            />
          )}
        </div>
      </div>
      {isAddDocumentVisible && (
        <AddDocumentChecklist
          ...
          fetchOffshoreDocuments={fetchOffshoreDocuments}
        />
      )}
...
export default OffshoreDocumentManagement;
